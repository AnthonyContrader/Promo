...
public class ClientDAO implements DAO<Client> {
		...
		private final String QUERY_CREATE = "INSERT INTO client (idclient, username, password, scode, email) VALUES (?,?,?,?,?)";
		private final String QUERY_READ = "SELECT * FROM client WHERE idclient=?";
		private final String QUERY_UPDATE = "UPDATE client SET username=?, password=?, scode=? email=? WHERE idclient=?";
		...
		public List<Client> getAll() {
			...
			try {
				...
				while (resultSet.next()) 
				{
					...
					client = new Client(username, password, scode, email);
		...
		public boolean insert(Client clientToInsert) {
			...
			try {	
				...
				preparedStatement.setString(1, clientToInsert.getUsername());
				preparedStatement.setString(2, clientToInsert.getPassword());
				preparedStatement.setString(3, clientToInsert.getScode());
				preparedStatement.setString(4, clientToInsert.getEmail());
		...
		public Client read(int clientId) {
			...
			try {
				...
				Client client = new Client(username, password, scode, email);
		...
		public boolean update(Client clientToUpdate) {
			...
			if (!clientRead.equals(clientToUpdate)) {
				try {
					// Fill the clientToUpdate object
					...
					preparedStatement.setString(1, clientToUpdate.getUsername());
					preparedStatement.setString(2, clientToUpdate.getPassword());
					preparedStatement.setString(3, clientToUpdate.getScode());
					preparedStatement.setString(4, clientToUpdate.getEmail());
					preparedStatement.setInt(5, clientToUpdate.getId());
